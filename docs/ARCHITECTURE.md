...
**B8, resolved (2026-09-17, `ilspycmd` against the real install, VERIFIED).** There is no discrete "siege stage" state machine or persisted stage field, contrary to what `MapEvent.CheckSiegeStageChange()`'s existence suggested to the Phase 0 audit. In fact that exact method is **dead code in v1.4.8**: it computes each side's healthy-member count and then does nothing with the result (`if (num == 0) { }` — empty body). That explains, rather than just correlates with, `VERSION_SUPPORT.md`'s independent finding that it was removed entirely in 1.5.3-beta — it was already vestigial before removal.

What actually stands in for "siege stage" is `SiegeEvent.GetCurrentBattleType()`, and it's purely derived, not stored: if the besieger's leader party currently has a live `MapEvent`, return that event's `EventType` (`SiegeAssault`, `SallyOut`, `Raid`, `BlockadeBattle`, ...); otherwise fall back to plain `Siege` (besieging, no battle happening right now). So "stage transitions" are really just **new `MapEvent`s being created with a particular `EventType`**, triggered from two places: player-initiated, through the same `MenuHelper`/game-menu-consequence layer B7 already found (e.g. `menu_siege_strategies_sally_out_from_port`); or AI-initiated, through `AiPartyThinkBehavior`'s `AiBehavior.BesiegeSettlement` decision loop. Siege *engineering* (construction/bombardment via `SiegeEvent.ConstructionTick`/`BombardTick`/`DoSiegeAction`) is a separate, parallel concern from which `BattleTypes` is currently active — a siege can sit in plain `Siege` for a long time while engines are built, with no "stage" changing until something (player or AI) actually triggers an assault/sally-out `MapEvent`.

**Not pursued further:** the exact AI heuristics for *when* `AiPartyThinkBehavior`/the AI utility models decide to trigger an assault. That's tuning-level detail relevant to reimplementing Bannerlord's AI, not to this project — we sync *outcomes* (via `CampaignEvents`/`MapEventEnded`), not AI decision-making itself.
